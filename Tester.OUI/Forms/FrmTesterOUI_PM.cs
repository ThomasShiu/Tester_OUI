...
{
    public partial class FrmTesterOUI_PM : Form
    {
        ...
        private void txtLotNo_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                try
                {
                    ...
                    //End lot command send to Handler
                    EndLot();
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Bin isobin = new Bin(UtilConstants.REJECT_ISO, 0);

                foreach (Bin bin in gH_HardBins.Values)
                {
                    if (bin.Qty == 0)
                        continue;

                    if (bin.Name.Contains("HardBin"))
                    {
                        string binnum = bin.Name.Substring(7, bin.Name.Length - 7);
                        if (int.Parse(binnum) == 1)
                        {
                            goodhardbinsQty = bin.Qty;
                        }
                        else
                        {
                            hardbins += "FTBIN" + binnum + "," + bin.Qty + ";";
                        }
                    }

                    if (bin.Name.Contains("FailMark"))
                    {
                        Bin tempbin = new Bin(UtilConstants.REJECT_INCORRECT_MARK, 0);
                        tempbin.Qty = bin.Qty;
                        hardbins += tempbin.Name + "," + tempbin.Qty + ";";
                    }

                    if (bin.Name.Contains("FailLead"))
                    {
                        Bin tempbin = new Bin(UtilConstants.REJECT_LEAD_REJECT, 0);
                        tempbin.Qty = bin.Qty;
                        hardbins += tempbin.Name + "," + tempbin.Qty + ";";
                    }

                    if (bin.Name.Contains("FailPackage"))
                    {
                        Bin tempbin = new Bin(UtilConstants.REJECT_PACKAGE, 0);
                        tempbin.Qty = bin.Qty;
                        hardbins += tempbin.Name + "," + tempbin.Qty + ";";
                    }

                    if (bin.Name.Contains("Iso"))
                    {
                        isobin.Qty += bin.Qty;
                    }
                }
                ...
                if (isobin.Qty > 0)
                    hardbins += isobin.Name + "," + isobin.Qty + ";";
        ...
        #region "PM Secs/Gem Communication"
        ...
        private void NewLot()
        {
            try
            {
                ...
                if (gv_WinSecs.Connected)
                {
                    ...
                    SECSItem commandItem = trans.Primary.Root.Item(0).Item(0);
                    commandItem.Format = SECS_FORMAT.ASCII;
                    commandItem.Value = "NEWLOT";

                    SECSItem lotitem = trans.Primary.Root.Item(0).Item(1).Item(0);
                    lotitem.Item(0).Format = SECS_FORMAT.ASCII;
                    lotitem.Item(0).Value = "LOT_NUMBER";
                    lotitem.Item(1).Format = SECS_FORMAT.ASCII;
                    lotitem.Item(1).Value = txtLotNo.Text.Trim();

                    SECSItem useritem = lotitem.Duplicate();
                    useritem.Item(0).Format = SECS_FORMAT.ASCII;
                    useritem.Item(0).Value = "OPERATOR_ID";
                    useritem.Item(1).Format = SECS_FORMAT.ASCII;
                    useritem.Item(1).Value = UtilCommonInfo.UserName;

                    SECSItem deviceitem = lotitem.Duplicate();
                    deviceitem.Item(0).Format = SECS_FORMAT.ASCII;
                    deviceitem.Item(0).Value = "DEVICE_ID";
                    deviceitem.Item(1).Format = SECS_FORMAT.ASCII;
                    deviceitem.Item(1).Value = lotEntity.DeviceId;

                    SECSItem qtyitem = lotitem.Duplicate();
                    qtyitem.Item(0).Format = SECS_FORMAT.ASCII;
                    qtyitem.Item(0).Value = "QUANTITY_IN";
                    qtyitem.Item(1).Format = SECS_FORMAT.U4;
                    qtyitem.Item(1).Value = lotEntity.LotQty;
        ...
        private void EndLot()
        {
            try
            {
                ...
                sTag = UtilConstants.END_LOT;

                if (gv_WinSecs.Connected)
                {
                    SECSTransaction trans = gv_HostLib.Find("S2F41");
                    trans.Tag = UtilConstants.END_LOT;
                    trans.Primary.Root.Item(0).Item(0).Value = "ENDLOT";
        ...
        private void HandleRemoteCommand(SECSTransaction trans)
        {
            try
            {
                ...
                if (ack != 0)
                {
                    ...
                    this.BeginInvoke(In);
                    if (sTag == UtilConstants.END_LOT)
                    { sMsg = "结束Hanlder上当前LOT失败！"; }

                    if (sTag == UtilConstants.NEW_LOT)
                    { sMsg = "发送Lot信息到Hanlder上失败！"; }
